...
export const sendMessageWithImage = async (mesageDetails) => {
  const { sender_id, groupid, image_url } = mesageDetails;
  ...
  try {
    ...
    if (image_url && image_url.length > 0) {
      for (const imageUri of image_url) {
        ...
        const fileName = `GroupMessage/${Date.now()}_${fileNameFromUri(imageUri)}`;
    ...
    const { data, error } = await supabase.from("GroupMessage").insert([message]);
